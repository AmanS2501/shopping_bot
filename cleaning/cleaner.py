...
class TextCleaner:
    ...
    def _compile_patterns(self):
        ...
        self.email_pattern = re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b')
        self.whitespace_pattern = re.compile(r'\s+')
        self.special_chars_pattern = re.compile(r"[^a-zA-Z0-9\s\.\,\!\?\:\'\"\-\(\)\[\]]")  # Semicolon removed here
        self.excessive_punct_pattern = re.compile(r'([.!?]){3,}')
        self.bullet_pattern = re.compile(r'^[\s]*[•·▪▫◦‣⁃*\-\+]\s*', re.MULTILINE)
        self.numbered_list_pattern = re.compile(r'^[\s]*\d+[\.\)]\s*', re.MULTILINE)
        self.header_pattern = re.compile(r'^[A-Z\s]{3,}$', re.MULTILINE)
    ...
    def normalize_spacing(self, text: str) -> str:
        # Replace newlines with space instead of preserving structure
        text = text.replace('\n', ' ')
        # Remove semicolons by replacing them with space
        text = text.replace(';', ' ')
        # Normalize horizontal whitespace to single spaces
        text = re.sub(r'[ \t]+', ' ', text)
        return text.strip()

    # filter_sentences removed along with calls
    ...
    def clean_text(self, text: str) -> str:
        ...
        text = self.special_chars_pattern.sub(' ', text)
        text = self.normalize_spacing(text)
        # Removed call to filter_sentences here
        text = text.strip()
        if not text:
            return ""
        return text
